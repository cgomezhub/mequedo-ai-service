...
- **Rate-limit storm hardening (marketing):** NVIDIA NIM free tier is **per-minute** (a single probe call can `200` while a full crew burst `429`s). Three stacked retry layers (litellm/SDK + CrewAI ReAct loop + `_kickoff_marketing_with_retry`) once amplified a transient 429 into a multi-minute hang that left the doc stuck at `status="processing"` (CrewAI's `max_execution_time` can't interrupt a blocking SDK retry-sleep). Mitigations: `num_retries=MARKETING_LLM_NUM_RETRIES` (default `1`) keeps the SDK retry layer shallow; `_kickoff_marketing_with_retry` backs off `MARKETING_RATE_LIMIT_BACKOFF` (default `30`s) on a real 429; and `_run_with_hard_timeout` caps the whole job at `MARKETING_JOB_TIMEOUT` (default `240`s) so it **always** reaches `draft`/`error`. The marketing views set `throttle_classes = []` (internal, `X-Internal-Secret`-guarded) so the poller isn't masked by DRF's `anon: 10/min` throttle. See `.agent/agent-logic-context/markenting_AIagents_flow_context.md` §12.
